...
The report uses XeLaTeX-compatible Unicode fonts and the \texttt{listings} package. Code is rendered on a black background with line numbers; the number column is italic and grey. No shell escape or network access is required.

\begin{lstlisting}[style=vstyle,language=bash,numbers=left,firstnumber=1]
...
The atlas source is \path{DEMO_control_charts_realscale.zip}. Appendix~\ref{app:atlas-data} lists every CSV and includes its values as numbered listings. The companion A4 PDF contains one chart per page.
